...
# Memoization cache: (node, dac_visited, fft_visited) -> number of paths
memo = {}

def count_paths(node, dac_visited, fft_visited):
    if node == "dac":
        dac_visited = True
    if node == "fft":
        fft_visited = True
    ...
    key = (node, dac_visited, fft_visited)
    if key in memo:
        return memo[key]

    total = 0
    for nxt in graph.get(node, []):
        total += count_paths(nxt, dac_visited, fft_visited)
